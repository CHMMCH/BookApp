...
public class BookPageView extends RelativeLayout {
    ...
    private void init(Context context) {
        ...
        setOnTouchListener(new OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                boolean isCenterOfX = event.getX() < mViewWidth/3*2 && event.getX() > mViewWidth/3;
                boolean isCenterOfY = event.getY() < mViewHeight/3*2 && event.getY() > mViewHeight/3;

                if(isCenterOfX && isCenterOfY){
                    if (event.getAction() == MotionEvent.ACTION_UP){
                        Log.d(TAG, "onTouch: center");
                        if(userTouchSettingListener != null){
                            userTouchSettingListener.userTouch();
                        }
                    }
                    return true;
                }
                ...
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_UP:
                        ...
                        if(userCloseSetting != null){
                            userCloseSetting.userColse();
                        }
                        break;
                }
                return false;
    ...
    private void drawNextPageAreaAndShadow(Canvas canvas, Bitmap bitmap) {
        ...
        if(Build.VERSION.SDK_INT >= 28){
            canvas.clipOutPath(mNextPagePath);
            canvas.clipOutPath(mCurrentPagePath);
        }else {
    ...
    public userTouchSetting userTouchSettingListener;

    public userTouchSetting getUserTouchSettingListener() {
        return userTouchSettingListener;
    }

    public void setUserTouchSettingListener(userTouchSetting userTouchSettingListener) {
        this.userTouchSettingListener = userTouchSettingListener;
    }

    public interface userTouchSetting{
        void userTouch();
    }

    private userCloseSetting userCloseSetting;

    public BookPageView.userCloseSetting getUserCloseSetting() {
        return userCloseSetting;
    }

    public void setUserCloseSetting(BookPageView.userCloseSetting userCloseSetting) {
        this.userCloseSetting = userCloseSetting;
    }

    public interface userCloseSetting{
        void userColse();
    }
}
